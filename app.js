...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchLiveRates() {
        try {
            // Using a free, reliable API for daily exchange and metal rates
            // Added cache-busting timestamp and 'no-store' to prevent mobile browsers from showing stale cached data
            const timestamp = new Date().getTime();
            let response;
            try {
                response = await fetch(`https://latest.currency-api.pages.dev/v1/currencies/usd.json?_t=${timestamp}`, { cache: 'no-store' });
            } catch (err) {
                // Fallback to jsdelivr if pages.dev fails
                response = await fetch(`https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/usd.json?_t=${timestamp}`, { cache: 'no-store' });
            }

            if (!response || !response.ok) throw new Error('Failed to fetch live rates');

            const data = await response.json();
            const usd = data.usd;

            if (usd && usd.pkr && usd.xau && usd.xag) {
                // Determine Live Ounce Prices from API (API returns inverse, e.g., 1 USD = 0.0003 XAU)
                const liveGoldOunceUSD = 1 / usd.xau;
                const liveSilverOunceUSD = 1 / usd.xag;

                // 1 Troy Ounce = 31.1034768 grams.
                const goldPerGramUSD = liveGoldOunceUSD / 31.1034768;
                const silverPerGramUSD = liveSilverOunceUSD / 31.1034768;
                ...
                if (timeEl) {
                    const now = new Date();
                    timeEl.innerText = 'آخری اپڈیٹ: ' + now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' (' + now.toLocaleDateString() + ')';
                }
            }
        } catch (error) {
